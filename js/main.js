...
function showTasksList() {
  ...
  if (list.length === 0) {
    ...
    clearCompleteTasksBtn.disabled = true;
  ...
  ls = list.filter((task) => task.completed);
  if (ls.length == 0) {
    clearCompleteTasksBtn.disabled = true;
  } else {
    clearCompleteTasksBtn.disabled = false;
  }
    ...
    const element = String.raw`
        <li class="ui segment grid equal width">
          ...
          <div class="column">
            ...
            <i data-id="${
              task.id
            }" class="trash alternate outline remove icon"></i>
          </div>
        </li>
      `;
...
// Update the event listener for the "Add Task" form
...
// Show Clear all completed task modal
function showClearCompletedTasksModal() {
  if (list.length > 0) {
    return $("#clear-completed-tasks-modal").modal("show");
  }
